...
>
> **탭4 승인 화면이 전부다.** **기본값은 탭1 + 탭4만(약 5분)이다** — 자기소개가 길어질 걸 감안해 데모 시간을 줄여뒀다. 탭2·탭3는 시간이 남을 때만 넣는 **선택 사항**이다(§2·§3 참조).
>
...
**기본 흐름은 여기서 탭4로 바로 넘어간다** — "차단 판정이 어떻게 기록되고 기준으로 돌아가는지는 탭4에서 한 번에 보여드리겠습니다"로 연결한다. 탭2·탭3은 시간이 남을 때만 연다.

---

## 2. 탭2 `Threat Events` — 증거가 남는다 (선택, 시간 남으면 +1분)
...
## 3. 탭3 `Risk Dashboard` — 이벤트가 기준으로 되돌아간다 (선택, 시간 남으면 +1분)
...
> **PIPA 자동화된 결정(REQUIRE_APPROVAL) 질문으로 왔다면** — 여기서 보여주는 "결정 지점(diff·승인)과 집행 지점(반영)이 분리돼 있고, 승인 전에는 멈춰 있다"는 구조가 `03_#1.1-1.5_keywords.md` 1.1절 "규제를 게이트 조건으로"에서 설명한 **PDP/PEP 분리**와 같은 형태다. **탭4는 정책 변경 승인이고 PIPA가 요구하는 건 개별 실행 승인이라 층위는 다르지만, "결정과 집행 사이에 사람 승인이 끼어들고 그 전엔 진행되지 않는다"는 같은 구조를 화면으로 보여준다.** 이 구분을 흐리지 않는다 — 물으면 "지금 화면은 정책 기준의 변경 승인이고, 개별 자동화된 결정에 대한 승인은 같은 구조를 런타임의 PDP/PEP 레벨에 적용한 것"이라고 명확히 한다.
...
| 순서 | 무엇 | 어디 |
|---|---|---|
| 1 | 다섯 계층 + Policy/Guardrail 구별 | `03_#1.1-1.5_keywords.md` 1.1절 |
| 2 | 역할 소유권 | §3 |
...
**시간이 남으면 탭1 이후에 추가 (+2분, 최대 7분)**

| 순서 | 화면 | 시간 | 목적 |
|---|---|---:|---|
| — | 탭2 이벤트 | +1분 | 증거가 남는다 |
| — | 탭3 위험 환산 | +1분 | 증거가 기준으로 돌아간다 |
...
4. **탭4에 시간을 남긴다.** 기본값은 탭1(1.5분)+탭4(3분)뿐이다 — 탭1에서 지체하면 정작 핵심(탭4)에서 시간이 없다.
