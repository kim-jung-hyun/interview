...
**시간 배분** (인사 포함 소개가 길어질 걸 감안해 재배분 — Q&A를 지킨다)

| 블록 | 시간 | 자료 |
|---|---|---|
| 자기소개 3종 (인사 포함) | 10~15분 | `01_introduction.md` |
| **산출물 구조 + 1.1~1.5** | **20분 (합계 — 구조 3분 고정 + JD 5개 평균 3.4분)** | `02_structure.md` §0 + 아래 큐카드 |
| 데모 | **5분 (탭1+탭4만 기본)** | `03_demo.md` |
| Q&A·역질문 | 나머지 (약 17~25분) | §7 |

**소개가 15분을 넘으면** 손실은 Q&A가 아니라 **데모(탭4만으로 3분)에서 흡수**한다.
...
**여는 그림 (말 앞에 먼저 띄운다)** — `02_structure.md` §1 상단 다이어그램(요청→Model→Harness→Policy→Guardrail→Tool실행, 전 단계가 Evidence로 모임)을 화면에 띄우고 한 문장으로 연다.
> "제 산출물이 실제로 어디에 들어가는지 그림으로 먼저 보여드리면, **산출물①은 Model~Guardrail의 설계 요건**이고 **산출물②는 Evidence·Assessment 쪽 요건**입니다. 지금부터 이 앞쪽부터 채워온 걸 말씀드리겠습니다."

→ **여기서 시간을 더 쓰지 않는다.** 매니저가 "Harness가 뭔가요" 등으로 용어를 물으면 → "그림의 두 번째 상자입니다. 형식 검증 단계인데, 상세는 뒤에 질문 주시면 표로 보여드리겠습니다" 한 문장으로 닫고 바로 아래 목표 문장으로 넘어간다.
...
>
> 우선순위 근거는 **NIST Profile의 통제 목표 7개**입니다. 제 자산을 대조해 보니 **4개는 부분이고 3개가 없습니다.** '부분'의 공통 원인은 하나입니다 — **정책 버전이 증거에 연결되지 않는다**는 것. 없는 것이 human approval 증거, 최소권한 negative test, 개인정보 masking입니다. **그 빈칸이 목록입니다.**
