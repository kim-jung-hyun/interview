...
>
> **화면 표시 순서 (1시간 압축판, 내가 말하는 순서 그대로).** **§0(기준선 + 내 산출물 표)만 오프닝 3~4분에 항상 띄운다.** 질문이 시스템 구조·Q3 쪽으로 오면 **§0 바로 다음에 실행 흐름 그림(`figures/architecture.svg`)을 먼저 그림으로 보여주고, 그 다음에 §1·§2(1.1 자료, Test/E2E 구분 포함)로 말로 설명한다.** 이어서 §3(1.2 자료) → §4·§5·§6·§7(1.5 자료) → §8(Q6 첫 90일) 순서로, **전부 질문이 그쪽으로 왔을 때만 편다** — 화면에서 뺀 게 아니라 말하는 순서대로 뒤로 보낸 것이다. 말하기 대본은 `00_note.md` §0-1(오프닝)·§0-2(물으면) 참조. **약어는 맨 뒤 부록으로 옮겼다** — 말하면서 필요할 때 검색해서 쓴다.
...
> "그림으로 먼저 보여드리면, 요청이 Model → Harness → Policy → Guardrail을 거쳐 실행되고, 모든 단계가 Evidence로 기록됩니다. **이 그림을 말로 풀면 다음과 같습니다."** → §1로 이어간다.
...
**먼저 그림으로 — 요청 하나가 이 파이프라인을 통과한다** (기호로 개념만, 상세 그림은 `figures/architecture.svg`)
...
**Model은 통제 계층이 아니다** — action 후보를 제안하는 대상이고, 실제로 통제하는 건 나머지 다섯(Harness~Assessment)이다. 그래서 표제가 "다섯 계층"이다.

**이제 각 상자가 무엇이고, 통제로서 어떻게 동작해야 하는지 순서대로 설명한다.**
...
### Test와 E2E Test는 왜 따로인가 (`07_system_architecture.md` §2 근거)
...
*(경계 테스트 실측 건수·실패 분류는 `../tech/` 자료다. 물으면 답하고, 먼저 꺼내지 않는다.)*
...
### 역할이 시스템에서 붙잡는 지점 (`07_system_architecture.md` §6 근거)

**역할이 책임을 지려면 그 역할이 붙잡을 지점이 시스템에 있어야 한다.** 기준선 순서로 읽는다.

| 기준선 | 역할 | 시스템에서 붙잡는 지점 |
|---|---|---|
| **기준** | **AI GRC ← 내 자리** | **Policy 정의 + 개발팀 가이드 + evidence·audit 구조 요건** |
| **구현** | Development Owner / AI System Owner | **Model · Harness · Guardrail 구현**, 승인된 통제로 운영 |
| **판정** | Control Owner / QE·TEVV Owner | **Guardrail 실행·모니터링**, **Test / E2E Test와 그 결과** |
| **판정** | Release Approver / Internal Audit | **Evidence + Test 결과 → Go/No-go**, 정책 버전 이력의 독립 검증 |
| **수용** | Risk Owner / Business Owner | **Evidence에 남은 residual risk**, 사업상 위험 판단 |

> "**책임 체계와 시스템 구조는 따로 만들 수 없다고 생각합니다.** 시스템에 붙잡을 지점이 없는 역할은 문서상의 역할입니다. 그래서 제 일이 기준을 쓰는 것에서 끝나지 않습니다 — 그게 SW 구조에 들어가게 가이드하고, **그 구조에서 Assurance가 판정할 증거가 나오게** 하는 것까지입니다."
...
| 모든 도구 action을 독립적으로 authorization한다 | **부분** — 귀속 오류 존재 |
| 실행 과정을 사후 추적할 수 있다 (출처→결정→정책→승인→action) | **부분** — 정책 버전이 증거에 안 묶여 있다 |
| 통제 변경 후 회귀를 방지한다 | **부분** — 테스트가 정책 버전과 미연동 |
